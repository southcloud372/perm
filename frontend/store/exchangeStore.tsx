...
import { ACCOUNTS, fallbackAccount, getWalletClient, publicClient } from '../onchain/client';
...
class ExchangeStore {
  ...
  loadMyOrders = async (trader: Address): Promise<OpenOrder[]> => {
    ...
    if (this.isLoadingOrders) return this.myOrders; // 已有请求时返回旧数据
    try {
  ...
  refresh = async (silent = false) => {
    try {
      ...
      if (this.account) {
        ...
        const [m, pos, newOrders] = await Promise.all([
          ...
          this.loadMyOrders(this.account) // 加载订单
        ]);
